...
| 形式 | 開く<br>(中を見る・取り出す) | 作る・書き換える | 補足 |
|---|:---:|:---:|---|
| ZIP | ○ | ○ | 新規作成、追加、削除、名前の変更、書庫の中での移動、フォルダーの作成。圧縮方式は 無圧縮 / Deflate / Deflate64 / BZip2 / LZMA / PPMd、ZIP64(1 つの書庫に 4GB 超、6 万件超)も読めます |
...
| 自己解凍書庫(`.exe`) | ○ | 新規のみ | 中身が ZIP のものと 7z のものを開けます(名前ではなく中身を見て判断)。開いている ZIP から新しく作れますが、**できあがったものの書き換えはしません** |
| NSIS 製インストーラー(`.exe`) | ○ | × | 書庫ではなくインストーラーですが、中のファイルを一覧して取り出せます |
| 分割された書庫(`.001` `.002` …) | ○ | 新規のみ | `.001` を開くと続きを自動でつないで読み、足りない番号があればその番号を挙げて知らせます。分割は書庫に限らずどんなファイルでもでき、**つなぎ直すためのプログラムも一緒に作ります**。既にある分割の書き換えはしません |
| 書庫の中の書庫 | ○ | ○ | 選ぶと専用のタブで開きます。何段でも辿れ、書き換えて保存すると親の書庫へ書き戻します |
| RAR | × | × | 未対応です(将来の検討課題) |
| LZH | × | × | 対応しません |
