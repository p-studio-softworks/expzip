...
public partial class MainWindow : Window
{
    ...
    /// <summary>
    /// 決まりに合っていない項目を直す (仕様書 11.3節の5)。
    /// </summary>
    /// <remarks>
    /// **選んだものだけを、確かめてから直す。**どれを直すかはダイアログで選ぶ。
    /// 既定では1つも選ばれておらず、適用の前にもう一度確かめる (無断上書きはしない)。
    /// </remarks>
    private async Task RuleFixAsync()
    {
        if (_cancellation is not null || Tab is not { } tab || tab.Audit is not { } audit)
        {
            return;
        }

        var dialog = new RuleFixDialog(this, CurrentAiOptions, audit, tab.Contents);

        if (dialog.ShowDialog() != true || dialog.Accepted.Count == 0)
        {
            return;
        }

        await RunRuleFixAsync(dialog.Accepted);
    }
    ...
    /// </summary>
    /// <remarks>
    /// **取り除くほうを先にする。**名前を変えてから取り除くと、取り除く先の
    /// パスが変わっていて当たらない。取り除いても、ほかの項目の場所は動かない。
    /// </remarks>
    private async Task RunRuleFixAsync(IReadOnlyList<RuleFix> fixes)
    {
        if (Contents is not { } contents || !TryGetPassword(out var password))
        {
            return;
        }

        var archivePath = contents.FilePath;
        var here = CurrentFolder?.FullPath ?? string.Empty;
        ...
        {
            if (fix.Kind == RuleFixKind.Remove)
            {
                if (fix.IsFolder)
                {
                    folders.Add(fix.Path);
                }
                else if (FindEntry(contents.Root, fix.Path) is { } entry)
                {
                    // 消す側は書庫の中の元の名前で指す。正規化した形では当たらない
                    files.Add(entry.SourceName);
                }
            }
            else if (fix.Kind == RuleFixKind.Rename)
            {
                var name = fix.NewName.Trim();
                var parent = fix.Parent;

                changes.Add(new PathChange(
                    fix.Path, parent.Length == 0 ? name : parent + "/" + name, fix.IsFolder));
            }
        }
        ...
        SetBusy(true);
        ProgressIndicator.IsIndeterminate = true;
        StatusMessage.Text = Strings.RuleFixWorking;
        ...
        try
        {
            if (files.Count > 0 || folders.Count > 0)
            {
                var result = await Task.Run(() => password is null
                    ? ZipArchiveWriter.Delete(archivePath, files, folders, cancellation.Token)
                    : ZipEncryptedWriter.Delete(
                        archivePath, files, folders, password, cancellation.Token));

                cancelled = result.Cancelled;
                removed = result.Deleted;
            }

            if (!cancelled && changes.Count > 0)
            {
                var result = await Task.Run(() => password is null
                    ? ZipArchiveWriter.Move(archivePath, changes, null, cancellation.Token)
                    : ZipEncryptedWriter.Move(
                        archivePath, changes, password, null, cancellation.Token));
        ...
        {
            MessageBox.Show(
                this, Strings.RuleFixFailed(ex.Message),
                AppName, MessageBoxButton.OK, MessageBoxImage.Warning);
        }
        finally
        {
        ...
        // 書庫が変わった。読み直して当て直す
        ForgetAudits();
        await OpenArchiveAsync(archivePath, here);

        StatusMessage.Text = cancelled
            ? Strings.RuleFixCancelled
            : Strings.RuleFixDone(removed, renamed);
    }

    /// <summary>書庫内パスからファイルを探す。</summary>
    private static ArchiveEntry? FindEntry(ArchiveFolder folder, string fullPath)
    {
        foreach (var file in folder.Files)
        {
        ...
        foreach (var child in folder.Folders)
        {
            if (FindEntry(child, fullPath) is { } found)
            {
                return found;
            }
        }
    ...
    private void RuleAuditItem_Click(object sender, RoutedEventArgs e)
    {
        ...
        var window = new RuleAuditWindow(
            this, audit, tab.FilePath, JumpToRulePath, () => _ = RuleFixAsync());
