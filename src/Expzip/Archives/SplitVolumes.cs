...
/// <remarks>
/// <para>
/// この形の分割は**形式ではなく、ただ切っただけ**。順に繋げば元のファイルに
/// 戻る (実物の3断片で、繋いだものが元の ZIP と SHA-256 まで一致することを
/// 確かめてある)。どの道具が切ったかを知る必要がない。
/// </para>
/// <para>
...
/// 別の仕組みのため。連結用プログラム (exe) を解析する道も採らない。道具ごとの
/// 独自形式で、追随できない。
/// </para>
/// </remarks>
internal static class SplitVolumes
{
    ...
    /// </summary>
    /// <remarks>
    /// 揃っていない断片をそのまま繋ぐと、途中で切れた書庫が出来上がる。中身が
